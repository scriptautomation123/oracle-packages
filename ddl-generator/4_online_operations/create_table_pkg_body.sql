-- =====================================================
-- CREATE TABLE PACKAGE BODY - Oracle 19c+ Enterprise Edition
-- Principal Database Engineer Package Implementation
-- Version: 1.0
-- =====================================================

-- Supporting Types
CREATE OR REPLACE TYPE column_def_array IS TABLE OF create_table_pkg.column_def;
/
CREATE OR REPLACE TYPE constraint_def_array IS TABLE OF create_table_pkg.constraint_def;
/
CREATE OR REPLACE TYPE partition_def_array IS TABLE OF create_table_pkg.partition_def;
/
CREATE OR REPLACE TYPE table_definition IS OBJECT (
    ...
    properties     create_table_pkg.table_properties,
...
-- Package Body
CREATE OR REPLACE PACKAGE BODY create_table_pkg AS
    ...
    BEGIN
        ...
        IF p_partitions IS NOT NULL AND p_partitions.COUNT > 0 THEN
            v_partition_clause := ' PARTITION BY RANGE (' || p_partitions(1).column_list || ') (';
            
            FOR i IN 1..p_partitions.COUNT LOOP
                IF i > 1 THEN
                    v_partition_clause := v_partition_clause || ',';
                END IF;
                v_partition_clause := v_partition_clause || CHR(10) || '  PARTITION ' || p_partitions(i).partition_name || 
                                     ' ' || p_partitions(i).values_clause;
                IF p_partitions(i).tablespace IS NOT NULL THEN
                    v_partition_clause := v_partition_clause || ' TABLESPACE ' || p_partitions(i).tablespace;
                END IF;
            END LOOP;
