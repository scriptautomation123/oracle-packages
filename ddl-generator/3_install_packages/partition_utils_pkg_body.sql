-- =====================================================
-- Oracle Partition Utilities Package Body
...
AS
    ...
    BEGIN
        partition_logger_pkg.log_operation(
            p_operation_type => 'UTILS_' || p_operation,
    ...
    BEGIN
        ...
        IF p_table_name IS NULL OR LENGTH(TRIM(p_table_name)) = 0 THEN
            RAISE_APPLICATION_ERROR(-20017, 'Table name cannot be null or empty');
        END IF;
        ...
        -- Enhanced performance analysis using available system views
        OPEN v_cursor FOR
            SELECT 
                p.partition_name,
                p.num_rows,
                ...
                CASE 
                    ...
                    WHEN p.blocks < 8 THEN 'SMALL'
                    WHEN p.blocks > 10000 THEN 'LARGE'
                    ELSE 'MEDIUM'
                END as size_category,
                CASE 
                    WHEN p.last_analyzed IS NULL THEN 'NEVER_ANALYZED'
                    WHEN p.last_analyzed < SYSDATE - p_days_back THEN 'STALE_STATS'
                    ELSE 'CURRENT'
                END as stats_status,
                -- Performance indicators based on available data
                CASE 
                    WHEN p.num_rows > 0 AND p.blocks = 0 THEN 'HIGH_FRAGMENTATION'
                    WHEN p.blocks > 0 AND p.num_rows = 0 THEN 'EMPTY_BLOCKS'
                    ELSE 'NORMAL'
                END as performance_indicator,
                'Enhanced analysis using system metadata' as note
            FROM user_tab_partitions p
            WHERE p.table_name = UPPER(p_table_name)
            ORDER BY p.blocks DESC;
            
        RETURN v_cursor;
    EXCEPTION
        WHEN OTHERS THEN
            log_utils_operation('PERFORMANCE_ANALYSIS', p_table_name, 'ERROR', SQLERRM);
            RAISE;
    ...
    FUNCTION find_unused_indexes(
        ...
        v_cursor SYS_REFCURSOR;
    BEGIN
        -- Note: V$ view access requires special privileges
        -- This simplified version provides basic index analysis
        OPEN v_cursor FOR
            SELECT 
                index_name,
                index_type,
                'V$ view access not available - cannot determine usage' as reason
            FROM user_indexes
            WHERE table_name = UPPER(p_table_name)
            AND index_type NOT IN ('LOB', 'IOT - TOP');  -- Exclude system indexes
    ...
    -- String and partition utility functions (adapted from manage_partition_pkg)
    FUNCTION parse_delimited_string(
        p_delimited_string IN VARCHAR2,
        p_element_index IN NUMBER,
        p_delimiter IN VARCHAR2
    ) RETURN VARCHAR2 IS
        v_string VARCHAR2(4000) := p_delimited_string;
        v_delimpos PLS_INTEGER := INSTR(p_delimited_string, p_delimiter);
        v_delimlen PLS_INTEGER := LENGTH(p_delimiter);
        v_nfields PLS_INTEGER := 1;
        v_loop CHAR(1) := 'Y';
        v_retval VARCHAR2(4000) := p_delimited_string;
    BEGIN
        log_utils_operation('PARSE_DELIMITED_STRING', 'UTILS', 'STARTED', 'Parsing delimited string');
        
        WHILE (v_delimpos > 0 AND v_loop = 'Y') LOOP
            v_retval := SUBSTR(v_string, 1, v_delimpos - 1);
            IF (v_nfields = p_element_index) THEN
                v_loop := 'N';
            ELSE
                v_string := SUBSTR(v_string, v_delimpos + v_delimlen);
                v_retval := v_string;
                v_nfields := v_nfields + 1;
                v_delimpos := INSTR(v_string, p_delimiter);
            END IF;
        END LOOP;
        
        IF ((v_delimpos = 0) AND (v_nfields < p_element_index)) THEN
            v_retval := p_delimiter;
        END IF;
        
        log_utils_operation('PARSE_DELIMITED_STRING', 'UTILS', 'SUCCESS', 'Successfully parsed string');
        RETURN v_retval;
        
    EXCEPTION
        WHEN OTHERS THEN
            log_utils_operation('PARSE_DELIMITED_STRING', 'UTILS', 'ERROR', SQLERRM);
            RETURN NULL;
    ...
    FUNCTION is_interval_partitioned(
        ...
        v_partitioned_flg VARCHAR2(3);
        v_interval VARCHAR2(4000);
    BEGIN
        log_utils_operation('IS_INTERVAL_PARTITIONED', p_table_name, 'STARTED', 'Checking interval partitioning');
        
        SELECT partitioned INTO v_partitioned_flg 
        FROM user_tables 
        WHERE table_name = UPPER(p_table_name);
        
        IF v_partitioned_flg = 'YES' THEN
            SELECT CASE 
                WHEN interval IS NULL OR interval = ' ' THEN 'NO' 
                ELSE 'YES' 
            END INTO v_interval
            FROM user_part_tables 
            WHERE table_name = UPPER(p_table_name);
            
            log_utils_operation('IS_INTERVAL_PARTITIONED', p_table_name, 'SUCCESS', 
                               CASE WHEN v_interval = 'YES' THEN 'Table has interval partitioning' ELSE 'Table does not have interval partitioning' END);
            RETURN (v_interval = 'YES');
        END IF;
        
        log_utils_operation('IS_INTERVAL_PARTITIONED', p_table_name, 'SUCCESS', 'Table is not partitioned');
        RETURN FALSE;
        
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            log_utils_operation('IS_INTERVAL_PARTITIONED', p_table_name, 'ERROR', 'Table not found');
            RETURN FALSE;
        WHEN OTHERS THEN
            log_utils_operation('IS_INTERVAL_PARTITIONED', p_table_name, 'ERROR', SQLERRM);
            RETURN FALSE;
    END is_interval_partitioned;
    
    FUNCTION get_interval_definition(
        p_table_name IN VARCHAR2
    ) RETURN VARCHAR2 IS
        v_interval_definition VARCHAR2(4000);
    BEGIN
        log_utils_operation('GET_INTERVAL_DEFINITION', p_table_name, 'STARTED', 'Getting interval definition');
        
        IF is_interval_partitioned(p_table_name) THEN
            SELECT interval INTO v_interval_definition 
            FROM user_part_tables 
            WHERE table_name = UPPER(p_table_name);
            
            log_utils_operation('GET_INTERVAL_DEFINITION', p_table_name, 'SUCCESS', 'Retrieved interval definition');
            RETURN v_interval_definition;
        ELSE
            log_utils_operation('GET_INTERVAL_DEFINITION', p_table_name, 'SUCCESS', 'No interval partitioning');
            RETURN NULL;
        END IF;
        
    EXCEPTION
        WHEN OTHERS THEN
            log_utils_operation('GET_INTERVAL_DEFINITION', p_table_name, 'ERROR', SQLERRM);
            RETURN NULL;
    END get_interval_definition;
    ...
    FUNCTION partition_exists(
        ...
        v_count NUMBER;
    BEGIN
        log_utils_operation('PARTITION_EXISTS', p_table_name, 'STARTED', 'Checking if partition exists');
        ...
        log_utils_operation('PARTITION_EXISTS', p_table_name, 'SUCCESS', 
                           CASE WHEN v_count > 0 THEN 'Partition exists' ELSE 'Partition does not exist' END);
        RETURN (v_count > 0);
        
    EXCEPTION
        WHEN OTHERS THEN
            log_utils_operation('PARTITION_EXISTS', p_table_name, 'ERROR', SQLERRM);
            RETURN FALSE;
    ...
    BEGIN
        log_utils_operation('GET_PARTITION_HIGH_VALUE_DATE', p_table_name, 'STARTED', 'Getting partition high value as date');
        ...
        IF v_high_value != 'MAXVALUE' THEN
            EXECUTE IMMEDIATE 'BEGIN :1 := ' || v_high_value || '; END;' USING OUT v_high_value_date;
            log_utils_operation('GET_PARTITION_HIGH_VALUE_DATE', p_table_name, 'SUCCESS', 'Retrieved high value date');
            RETURN v_high_value_date;
        ELSE
            log_utils_operation('GET_PARTITION_HIGH_VALUE_DATE', p_table_name, 'SUCCESS', 'High value is MAXVALUE');
            RETURN NULL;
        END IF;
        
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            log_utils_operation('GET_PARTITION_HIGH_VALUE_DATE', p_table_name, 'ERROR', 'Partition not found');
            RETURN NULL;
        WHEN OTHERS THEN
            log_utils_operation('GET_PARTITION_HIGH_VALUE_DATE', p_table_name, 'ERROR', SQLERRM);
            RETURN NULL;
    ...
    PROCEDURE manage_interval_partitioning(
        ...
        v_sql VARCHAR2(4000);
    BEGIN
        log_utils_operation('MANAGE_INTERVAL_PARTITIONING', p_table_name, 'STARTED', 'Managing interval partitioning');
        
        IF UPPER(p_action) = 'DISABLE' THEN
            v_sql := 'ALTER TABLE ' || p_table_name || ' SET INTERVAL()';
        ELSIF UPPER(p_action) = 'ENABLE' AND p_interval_definition IS NOT NULL THEN
            v_sql := 'ALTER TABLE ' || p_table_name || ' SET INTERVAL(' || p_interval_definition || ')';
        ELSIF UPPER(p_action) = 'SET' AND p_interval_definition IS NOT NULL THEN
            v_sql := 'ALTER TABLE ' || p_table_name || ' SET INTERVAL(' || p_interval_definition || ')';
        ELSE
            RAISE_APPLICATION_ERROR(-20001, 'Invalid action or missing interval definition');
        END IF;
        
        EXECUTE IMMEDIATE v_sql;
        
        log_utils_operation('MANAGE_INTERVAL_PARTITIONING', p_table_name, 'SUCCESS', 
                           'Successfully ' || LOWER(p_action) || 'd interval partitioning');
                           
    EXCEPTION
        WHEN OTHERS THEN
            log_utils_operation('MANAGE_INTERVAL_PARTITIONING', p_table_name, 'ERROR', SQLERRM);
            RAISE;
    END manage_interval_partitioning;
